...
export class TasksService {
  ...
  async create(id: string, payload: CreateTaskDto) {
    ...
    try {
      const data = {
        ...payload,
        incentive: convertToKobo(Number(payload.incentive)),
        user_id: id,
      };
  ...
  async acceptTask(userId: string, id: string) {
    try {
      ...
      // NOTIFICATION: Notify Poster
      const posterId = task.user_id.toString();
      const acceptor = await this.usersService.findUserByID(userId);
      await this.notificationsService.sendPushNotification(
        posterId,
        'Task Accepted',
        `${acceptor.first_name} has accepted your task: ${task.task}`,
        { taskId: id, type: 'TASK_ACCEPTED' },
      );
  ...
  async completeTask(userId: string, id: string) {
    try {
      ...
      // NOTIFICATION: Notify Poster
      const posterId = task.user_id.toString();
      const executor = await this.usersService.findUserByID(userId);
      await this.notificationsService.sendPushNotification(
        posterId,
        'Task Completed',
        `Executor has marked your task as completed. Please review.`,
        { taskId: id, type: 'TASK_COMPLETED' },
      );
      return SUCCESS;
    } catch (err) {
      console.error(`Error while accepting task`, err);
  ...
  async acknowledgeTask(userId: string, id: string) {
    try {
      ...
      try {
        if (task.payment_method === PaymentMethod.CASH) {
          // CASH FLOW
          // 1. Do NOT transfer funds between users.
          // 2. Just record Debt for Executor (Commission logic can be refined later, e.g. 10% of incentive)
          // For now, let's assume debt = 10% of incentive? Or full amount?
          // Requirement says "track debt". Let's track 10% commission.
          const commission = Math.floor(task.incentive * 0.1);
          if (commission > 0) {
            await this.walletService.recordDebt(task.acceptedBy, commission);
          }

          // Notification to Executor
          await this.notificationsService.sendPushNotification(
            task.acceptedBy.toString(),
            'Payment Approved',
            `Poster has approved. Please collect cash. Commission of ₦${commission / 100} recorded.`,
            { taskId: id, type: 'PAYMENT_APPROVED_CASH' },
          );
        } else {
          // IN-APP FLOW
          ...
          const amount_kobo = Number(task.incentive); // Ensure number
          ...
          await this.notificationsService.sendPushNotification(
            task.acceptedBy.toString(),
            'Payment Received',
            `You have received ₦${task.incentive} in your wallet!`,
            { taskId: id, type: 'PAYMENT_RECEIVED' },
          );
        }
  ...
  async update(id: string, userId: string, updateTaskDto: UpdateTaskDto) {
    ...
    if (!existing) {
      throw new NotFoundException(
        `Task with id ${id} not found or you don't have permission to edit it`,
      );
    }
    ...
    if (
      payload.incentive !== undefined &&
      payload.incentive !== null &&
      payload.incentive !== ''
    ) {
